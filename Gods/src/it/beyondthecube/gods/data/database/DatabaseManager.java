...
import it.beyondthecube.gods.data.blocks.PlacedBlock;
import it.beyondthecube.gods.data.config.ConfigManager;
import it.beyondthecube.gods.data.config.MySQLConfig;
import it.beyondthecube.gods.data.database.DatabaseException.DatabaseExceptionType;
...
import it.beyondthecube.gods.gods.altar.Altar;
...
public class DatabaseManager 
{
	...
	public static void testDB() throws DatabaseException
	{
		try
		{
			...
			r=new SQLQuery("select * from £.thrones;",msqlc).excecuteQuery();
			int cnt=0;
			while(r.next())
			{
				cnt++;
			...
			if(cnt<4)
			{
				throw new DatabaseException("Exception",DatabaseExceptionType.NOT_COMPLETE);
			}
			r=new SQLQuery("select * from £.thrones;",msqlc).excecuteQuery();
			r=new SQLQuery("select * from £.thrones;",msqlc).excecuteQuery();
			r=new SQLQuery("select * from £.thrones;",msqlc).excecuteQuery();
		...
		{
			throw new DatabaseException("Exception",DatabaseExceptionType.NOT_FOUND);
		} 
		catch (SQLException e) 
		{
			throw new DatabaseException("Exception",DatabaseExceptionType.CONNECTION);
		}
	}
	public static FollowerData getFollower(OfflinePlayer p) 
	{
		...
		{
			if(p.isOnline()) p.getPlayer().sendMessage(Utility.formattedMessage(Utility.getMessage("msg.followers.loadfail")));
		}
		return null;
	}
	...
	{
		String s=new String("update £.gods set champ="+((p==null)?"NULL":"'"+p.getName()+"'")+" where idGod="+(new Integer(Utility.getDBNumberFromGodType(g.getGodType()))));
	...
	public static void loadAltars()
	{
		...
		try 
		{
			r = (new SQLQuery(new String("select a.god,a.type,l.x,l.y,l.z,l.world,a.id,a.owner from £.altar a, £.locations l where a.location=l.id;"),msqlc)).excecuteQuery();
		} 
		catch (MySQLSyntaxErrorException e1) 
		{
			e1.printStackTrace();
		...
		try 
		{
			if(r!=null)
			{
				while(r.next())
				{
					AltarManager.loadAltar(GodManager.getGod(Utility.getGodTypeFromDBNumber(r.getInt(1))),AltarType.valueOf(r.getString(2)),new Location(Bukkit.getServer().getWorld(r.getString(6)),r.getInt(3),r.getInt(4),r.getInt(5)), r.getInt(7),Bukkit.getPlayer(r.getString(8)));
				}
			}
		}
		catch(SQLException e)
	...
	public static void loadThrones() 
	{
		for(GodType t:GodType.values())
		{
			...
			{
				String str="select l.x,l.y,l.z,g.hasThrone,l.world from £.thrones g,£.locations l where g.location=l.id and god="+Utility.getDBNumberFromGodType(t)+";";
				ResultSet r=null;
				try
				{
	...
	public static void connectToDB() throws DatabaseException 
	{
		...
		{
			throw new DatabaseException("",DatabaseExceptionType.CONNECTION);
		}
	}
	public static void setPriest(OfflinePlayer player, boolean b) 
	{
		(new SQLQuery("update table £.followers set priest="+((Boolean) b).toString()+" where follower like"+player.getName(), msqlc)).excecuteUpdate();
	}
	public static OfflinePlayer getNewChampCandidate(GodType g)
	{
		try 
		{
			ResultSet r=(new SQLQuery("select follower from £.followers where reputation>999 and god="+Utility.getDBNumberFromGodType(g)+" order by reputation,points,follower limit 1",msqlc)).excecuteQuery();
			while(r.next())
			{
				return Bukkit.getOfflinePlayer(r.getString(1));
			}
			return null;
		} 
		catch (MySQLSyntaxErrorException e) 
	...
	public static void removeFollower(String name) 
	{
		try
		{
			ResultSet r=(new SQLQuery("select id from £.followers where follower like '"+name+"';",msqlc)).excecuteQuery();
			r.next();
		    new SQLQuery("delete from £.followers where id="+r.getInt(1)+";",msqlc).excecuteUpdate();					
		}
		catch(SQLException e)
		{
			e.printStackTrace();
